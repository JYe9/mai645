...
class acLSTM(nn.Module):
    ...
    def calculate_loss(self, out_seq, groundtruth_seq):
        ...
        loss_function = nn.L1Loss() # L1 loss
        loss = loss_function(out_seq, groundtruth_seq)
        return loss

    # # Angle Distance Loss
    # def calculate_loss(self, out_seq, groundtruth_seq):
    #     # Reshape the sequences to have dimensions (batch_size, seq_len, num_joints, 3)
    #     out_seq = out_seq.view(-1, Seq_len, Joints_num, 3)
    #     groundtruth_seq = groundtruth_seq.view(-1, Seq_len, Joints_num, 3)
    #
    #     # Compute the cosine similarity between the predicted and ground truth angles
    #     cos_error = torch.nn.CosineSimilarity(dim=-1)(out_seq, groundtruth_seq)
    #
    #     # Compute the angle distance loss
    #     angle_distance_loss = torch.mean(1 - cos_error)
    #
...
read_weight_path = ""
write_weight_folder = "../train_weight_aclstm_martial_L1_loss/"
# write_weight_folder = "../train_weight_aclstm_indian/"
# write_weight_folder = "../train_weight_aclstm_salsa/"

write_bvh_motion_folder = "../train_tmp_bvh_aclstm_martial_L1_loss/"
...
dance_frame_rate = 120 # 60 for salsa, 120 for martial and indian
batch = 32
in_frame = 171
out_frame = 171
hidden_size = 1024
