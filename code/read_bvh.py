...
# standard_bvh_file="C:/00_Assignment/MAI645_ML_for_CV/project645/train_data_bvh/standard.bvh"
...
## New Added
# def write_euler_to_bvh(euler_motion, skeleton, non_end_bones, format_filename, output_filename):
#     bvh_vec_length = len(non_end_bones) * 3 + 6
#
#     out_data = np.zeros([len(euler_motion), bvh_vec_length])
#     for i in range(1, len(euler_motion)):
#         euler_angles = euler_motion[i]
#
#         # Convert Euler angles to rotation matrices
#         rotation_matrices = {}
#         for joint in non_end_bones:
#             offset = joint_index[joint] * 3
#             rotation_matrices[joint] = euler.euler2mat(euler_angles[offset], euler_angles[offset + 1],
#                                                        euler_angles[offset + 2])
#
#         # Convert rotation matrices to BVH format
#         new_motion = helper.rotation_dic_to_vec(rotation_matrices, non_end_bones, euler_angles[:3])
#
#         new_motion = np.array([round(a, 6) for a in new_motion])
#         new_motion[0:3] = euler_angles[0:3]
#
#         out_data[i, :] = np.transpose(new_motion[:, np.newaxis])
#
#     write_frames(format_filename, output_filename, out_data)
...
def write_euler_to_bvh(euler_motion, skeleton, non_end_bones, format_filename, output_filename):
    ...
    for i in range(1, len(euler_motion)):
        ...
        # Convert Euler angles to rotation matrices
        rotation_matrices = {}
        print('skeleton:', skeleton, euler_angles.shape)
        offset = 0
        for joint in skeleton:
            if len(skeleton[joint]['channels']) == 0:
                continue

            print(skeleton[joint]['channels'])

            rotation_matrices[joint] = euler.euler2mat(euler_angles[offset], euler_angles[offset + 1], euler_angles[offset + 2])
        ...
        # Convert rotation matrices to BVH format
        hip_position = {'hip': euler_angles[:3]}  # Pass the hip position as a dictionary
        new_motion = helper.rotation_dic_to_vec(rotation_matrices, non_end_bones, hip_position)
...
#         data = train_data[i]
#         data = np.array([round(a, 6) for a in train_data[i]])
#         euler_motion.append(data)
#
#     write_euler_to_bvh(euler_motion, skeleton, non_end_bones, format_filename, bvh_filename)

def write_euler_traindata_to_bvh(bvh_filename, train_data):
    """
    Writes Euler angle training data to a BVH file.

    :param bvh_filename: Output BVH file path.
    :param train_data: Training data as a NumPy array of shape (num_frames, Joints_num*3 + 3).
    """
    try:
        seq_length = train_data.shape[0]
        euler_motion = []
        ...
        for i in range(seq_length):
            data = train_data[i]
            data = np.array([round(a, 6) for a in data])  # Ensure 6 decimal precision
            euler_motion.append(data)

        # Load standard BVH file to get header information
        with open(standard_bvh_file, 'r') as file:
            bvh_header = []
            line = file.readline()
            while "MOTION" not in line:
                bvh_header.append(line)
                line = file.readline()
            bvh_header.append(line)
            # Skip the next two lines in the header which are "Frames: " and "Frame Time: "
            bvh_header.append(file.readline())  # Frames line
            bvh_header.append(file.readline())  # Frame Time line

        with open(bvh_filename, 'w') as file:
            for line in bvh_header:
                file.write(line)
            file.write(f"Frames: {seq_length}\n")
            file.write("Frame Time: 0.033333\n")  # Assuming 30 FPS

            for frame in euler_motion:
                frame_data = " ".join(map(str, frame))
